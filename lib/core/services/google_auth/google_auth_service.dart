...
import 'package:supabase_flutter/supabase_flutter.dart' as supabase show AuthResponse, OAuthProvider;
import '../../errors/failures.dart' as app_errors;
...
class GoogleAuthService {
  ...
  Future<supabase.AuthResponse> signIn() async {
    try {
      ...
      if (account == null) {
        throw app_errors.AuthException('Sign-in cancelled.', code: 'cancelled');
      }
      ...
      if (idToken == null) {
        throw app_errors.AuthException('Google did not return an identity token.', code: 'no_id_token');
      }
      ...
      throw app_errors.AuthException('Google sign-in failed: $e', code: 'google_sign_in_error');
    }
  }
